...
/* Function to write a 32-bit unsigned integer in big-endian format to a byte array */
...
static bool els_is_active_keyslot(mcuxClEls_KeyIndex_t keyIdx)
{
...
{
    status_t status = (int32_t)STATUS_SUCCESS;
                         ...
                         size_t *output_size)
{
    ...
    *output_size = length_bytes;
    ...
    if (status != (int32_t)STATUS_SUCCESS)
    {
        mbedtls_cipher_free(&ctx);
    }
                                   ...
                                   mcuxClEls_KeyIndex_t *index_output)
{
    ...
    /* CKDF with mbedTLS */
    status = host_derive_key(shared_secret, s_CkdfDerivationDataWrapIn, s_WrapInKeyProp.word.value, &key_wrap_in[0],
                             &key_wrap_in_size);

    /* Import 256bit key into ELS key store */
    status =
        create_els_import_keyblob(plain_key, key_properties, key_wrap_in, &els_key_in_blob[0], &els_key_in_blob_size);
    ...
    /* If status != STATUS_SUCCESS, delete every key generated */
    if (status != (int32_t)STATUS_SUCCESS)
    {
        for (size_t i = 0U; i < ARRAY_SIZE(potentially_used_key_indices); ++i)
        {
