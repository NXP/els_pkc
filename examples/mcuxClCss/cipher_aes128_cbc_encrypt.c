 ...
 * @brief Example AES-128 CBC encryption using the CSSv2 (CLNS component mcuxClCss
 *
 * @example cipher_aes128_cbc_encrypt.c
 * @brief   Example AES-128 CBC encryption using the CSSv2 (CLNS component mcuxClCss)
 */
...
static uint8_t const aes128_input[MCUXCLCSS_CIPHER_BLOCK_SIZE_AES] = {0x6BU, 0xC1U, 0xBEU, 0xE2U,
                                                                     0x2EU, 0x40U, 0x9FU, 0x96U,
                                                                     0xE9U, 0x3DU, 0x7EU, 0x11U,
                                                                     0x73U, 0x93U, 0x17U, 0x2AU};

/** IV of the AES encryption. */
static uint8_t aes128_iv[MCUXCLCSS_CIPHER_BLOCK_SIZE_AES] = {0xF8U, 0xD2U, 0x68U, 0x76U,
                                                            0x81U, 0x6FU, 0x0FU, 0xBAU,
                                                            0x86U, 0x2BU, 0xD8U, 0xA3U,
                                                            0x2DU, 0x04U, 0x67U, 0xC3U};

/** Expected ciphertext output of the AES encryption. */
static uint8_t const aes128_expected_output[MCUXCLCSS_CIPHER_BLOCK_SIZE_AES] = {0xCAU, 0xEAU, 0x07U, 0x26U,
                                                                               0x62U, 0xE2U, 0x20U, 0x06U,
                                                                               0x2DU, 0x45U, 0x46U, 0x41U,
                                                                               0x5EU, 0xFFU, 0xFAU, 0xD2U};

/** Key for the AES encryption. */
static uint32_t const aes128_key[MCUXCLCSS_CIPHER_KEY_SIZE_AES_128 / sizeof(uint32_t)] = {0x16157E2B, 0xA6D2AE28, 0x8815F7AB, 0x3C4FCF09};
...
bool cipher_aes128_cbc_encrypt(
    void)
{
    ...
    mcuxClCss_CipherOption_t cipher_options = {0U};                        // Initialize a new configuration for the planned mcuxClCss_Cipher_Async operation.
    cipher_options.bits.cphmde = MCUXCLCSS_CIPHERPARAM_ALGORITHM_AES_CBC;  // Configure the AES block cipher mode of operation to be the CBC (Cipher Block Chaining) mode.
    cipher_options.bits.dcrpt = MCUXCLCSS_CIPHER_ENCRYPT;                  // Configure that the mcuxClCss_Cipher_Async operation shall perform  encryption.
    cipher_options.bits.extkey = MCUXCLCSS_CIPHER_EXTERNAL_KEY;            // Configure that an external key should be used.
    
    MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(result, token, mcuxClCss_Cipher_Async(   // Perform the encryption.
            ...
            (const uint8_t *) aes128_key, MCUXCLCSS_CIPHER_KEY_SIZE_AES_128,   // The AES key for the encryption (external key).
    ...
    MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(result, token, mcuxClCss_WaitForOperation(MCUXCLCSS_ERROR_FLAGS_CLEAR)); // Wait for the mcuxClCss_Cipher_Async operation to complete.
