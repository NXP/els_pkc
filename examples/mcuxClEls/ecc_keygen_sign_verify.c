...
bool ecc_keygen_sign_verify(void)
{
    ...
    {
        return false; // Expect that no error occurred, meaning that the mcuxClEls_EccSign_Async operation was started.
    }
    MCUX_CSSL_FP_FUNCTION_CALL_END();
    ...
    {
        return false; // Expect that no error occurred, meaning that the mcuxClEls_EccVerify_Async operation was started.
    }
    MCUX_CSSL_FP_FUNCTION_CALL_END();
    ...
    {
        return false; // Expect that mcuxClEls_EccVerify_Async operation successfully performed the signature verification.
    }
