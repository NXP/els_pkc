...
MCUXCLCSS_API MCUX_CSSL_FP_PROTECTED_TYPE(mcuxClCss_Status_t) mcuxClCss_Ckdf_Sp800108_Async(
    ...
    uint8_t const * pDerivationData)
{
    ...
    mcuxClCss_setInput0_fixedSize(pDerivationData);
...
MCUXCLCSS_API MCUX_CSSL_FP_PROTECTED_TYPE(mcuxClCss_Status_t) mcuxClCss_Hkdf_Rfc5869_Async(
...
MCUXCLCSS_API MCUX_CSSL_FP_PROTECTED_TYPE(mcuxClCss_Status_t) mcuxClCss_Hkdf_Sp80056c_Async(
    mcuxClCss_KeyIndex_t derivationKeyIdx,
    uint8_t * pTagetKey,
    uint8_t const * pDerivationData,
    size_t derivationDataLength)
{
    ...
    MCUXCLCSS_INPUT_PARAM_CHECK_PROTECTED(mcuxClCss_Hkdf_Sp80056c_Async, (CSS_KS_CNT <= derivationKeyIdx) );

    /* CSS SFRs are not cached => Tell SW to wait for CSS to come back from BUSY state before modifying the SFRs */
    if (mcuxClCss_isBusy())
    {
        MCUX_CSSL_FP_FUNCTION_EXIT(mcuxClCss_Hkdf_Sp80056c_Async, MCUXCLCSS_STATUS_SW_CANNOT_INTERRUPT);
    }
    ...
    mcuxClCss_setKeystoreIndex0(derivationKeyIdx);
    mcuxClCss_setInput0(pDerivationData, derivationDataLength);
    mcuxClCss_setOutput_fixedSize(pTagetKey);
    mcuxClCss_startCommand(ID_CFG_CSS_CMD_HKDF, options.word.value, CSS_CMD_BIG_ENDIAN);
...
MCUXCLCSS_API MCUX_CSSL_FP_PROTECTED_TYPE(mcuxClCss_Status_t) mcuxClCss_TlsGenerateMasterKeyFromPreMasterKey_Async(
    uint8_t const * pDerivationData,
...
MCUXCLCSS_API MCUX_CSSL_FP_PROTECTED_TYPE(mcuxClCss_Status_t) mcuxClCss_TlsGenerateSessionKeysFromMasterKey_Async(
    uint8_t const * pDerivationData,
    mcuxClCss_KeyProp_t keyProperties,
    mcuxClCss_KeyIndex_t keyIdx)
{
