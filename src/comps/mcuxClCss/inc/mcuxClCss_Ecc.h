 ...
 * @{
 */
#define MCUXCLCSS_ECC_HASHED                     ((uint32_t) 0U) ///< Set this option at #mcuxClCss_EccSignOption_t.echashchl or #mcuxClCss_EccVerifyOption_t.echashchl to specify input is the hash of the message
#define MCUXCLCSS_ECC_NOT_HASHED                 ((uint32_t) 1U) ///< Set this option at #mcuxClCss_EccSignOption_t.echashchl or #mcuxClCss_EccVerifyOption_t.echashchl to specify input is the plain message

#define MCUXCLCSS_ECC_RTF                        ((uint32_t) 1U) ///< Set this option at #mcuxClCss_EccSignOption_t.signrtf to include the RTF in the signature
#define MCUXCLCSS_ECC_NO_RTF                     ((uint32_t) 0U) ///< Set this option at #mcuxClCss_EccSignOption_t.signrtf to not include the RTF in the signature
/**
 * @}
 */
...
/**
 * @defgroup MCUXCLCSS_INTERNAL_VALUE_BITS Option bit field values that are needed for internal use only
 * @brief Internal bit field constants for several option types.
 * @ingroup mcuxClCss_Ecc_Macros
 * @{
 */
#define MCUXCLCSS_ECC_REVERSEFETCH_ENABLE        ((uint32_t) 1U) ///< Reverse Fetch enabled. For internal use
#define MCUXCLCSS_ECC_REVERSEFETCH_DISABLE       ((uint32_t) 0U) ///< Reverse Fetch disabled. For internal use

#ifdef MCUXCL_FEATURE_CSS_PUK_INTERNAL_BIT
#define MCUXCLCSS_ECC_EXTKEY_EXTERNAL        ((uint32_t) 1U) ///< Public key is taken from system memory. For internal use
#define MCUXCLCSS_ECC_EXTKEY_INTERNAL        ((uint32_t) 0U) ///< Public key is taken from internal keystore. For internal use
...
/**
 * @brief Command option bit field for #mcuxClCss_EccKeyExchange_Async, for internal use only.
 * Bit field to configure #mcuxClCss_EccKeyExchOption_t.
 */
typedef union
{
    struct
 ...
 * @retval #MCUXCLCSS_STATUS_OK_WAIT             on successful request
 */
MCUX_CSSL_FP_FUNCTION_DECL(mcuxClCss_EccKeyExchangeInt_Async)
 ...
 * @retval #MCUXCLCSS_STATUS_OK_WAIT             on successful request
 */
MCUX_CSSL_FP_FUNCTION_DECL(mcuxClCss_EccVerifyInt_Async)
