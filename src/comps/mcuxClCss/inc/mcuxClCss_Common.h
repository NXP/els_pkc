...
#define MCUXCLCSS_HW_VERSION ((mcuxClCss_HwVersion_t) { \
        .bits = { \
            .revision = (uint32_t) MCUXCLCSS_HW_VERSION_REVISION, \
            .minor = (uint32_t) MCUXCLCSS_HW_VERSION_MINOR, \
            .major = (uint32_t) MCUXCLCSS_HW_VERSION_MAJOR, \
            .fw_revision = (uint32_t) MCUXCLCSS_HW_VERSION_FW_REVISION, \
            .fw_minor = (uint32_t) MCUXCLCSS_HW_VERSION_FW_MINOR, \
            .fw_major = (uint32_t) MCUXCLCSS_HW_VERSION_FW_MAJOR \
        } \
    })

#ifdef MCUXCL_FEATURE_CSS_DMA_FINAL_ADDRESS_READBACK
  #define MCUXCLCSS_DMA_READBACK_PROTECTION_TOKEN MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClCss_CompareDmaFinalOutputAddress)
#else
  #define MCUXCLCSS_DMA_READBACK_PROTECTION_TOKEN (0u)
#endif
...
/**
 * @defgroup MCUXCLCSS_LOCKING_ MCUXCLCSS_LOCKING_
 * @brief Constants for CSS locking feature
 * @ingroup mcuxClCss_Common_Macros
 * @{ */
#define MCUXCLCSS_MASTER_UNLOCK_ANY         ((uint32_t) 0x1Fu) ///< Any bus master ID can override CSS lock
/** @} */
...
typedef union
{
    ...
    struct
    {
        ...
        uint32_t :3;            ///< RFU
#ifdef MCUXCL_FEATURE_CSS_LOCKING
        uint32_t css_locked :1; ///< CSS is locked
#else
        uint32_t :1;            ///< RFU
#endif /* MCUXCL_FEATURE_CSS_LOCKING */
        uint32_t :15;           ///< RFU
 ...
 * @else
 *  @return An error code that is always #MCUXCLCSS_STATUS_OK_WAIT
 * @endif
 */
...
#ifdef MCUXCL_FEATURE_CSS_LOCKING
/**
 * @brief Lock CSS to a session
 *
 * This operation locks the CSS, when the lock is obtained a nonzero value will be stored in @p sessionId, and the PPROT and bus Master ID
 * of the locking command will be recorded. Subsequent unlocking can only be done by one of the following:
 *      1.  CSS reset
 *      2.  Normal unlock: calling #mcuxClCss_ReleaseLock with the correct @p sessionId, from the same bus Master ID and using the same PPROT settings.
 *      3.  Privileged unlock: calling #mcuxClCss_ReleaseLock with any value of @p sessionId, from the bus Master ID set with the #mcuxClCss_SetMasterUnlock
 *          command and PPROT settings 'secure privileged'.
 * While CSS is locked read access is only permitted to CSS_STATUS and CSS_SESSION_ID and writes are only permitted to CSS_RESET, except for accesses that
 ...
 *
 * @retval #MCUXCLCSS_STATUS_OK                  if the operation was successful
 * @retval #MCUXCLCSS_STATUS_SW_LOCKING_FAILED   if the operation was not successful
 *
 * @if (MCUXCL_FEATURE_CSSL_FP_USE_SECURE_COUNTER && MCUXCL_FEATURE_CSSL_SC_USE_SW_LOCAL)
 *  @return A code-flow protected error code (see @ref mcuxCsslFlowProtection). The error code can be any error code in @ref MCUXCLCSS_STATUS_, see individual documentation for more information
 * @else
 ...
 * @endif
 */
MCUX_CSSL_FP_FUNCTION_DECL(mcuxClCss_GetLock)
MCUXCLCSS_API MCUX_CSSL_FP_PROTECTED_TYPE(mcuxClCss_Status_t) mcuxClCss_GetLock(
    uint32_t * pSessionId
    );

/**
 * @brief Release CSS lock for the session
 *
 * This operation unlocks the CSS when following conditions are met:
 *      1.  Normal unlock: providing the correct @p sessionId, from the same bus Master ID and using the same PPROT settings.
 *      2.  Privileged unlock: providing any value of @p sessionId, from the bus Master ID set with the #mcuxClCss_SetMasterUnlock command
 *          and PPROT settings 'secure privileged'.
 * Invalid attempts to unlock CSS will result in a bus error.
 *
 * @param[in] sessionId  The session identifier obtained while locking CSS
 *
 * @retval #MCUXCLCSS_STATUS_OK   if the operation was successful
 *
 * @if (MCUXCL_FEATURE_CSSL_FP_USE_SECURE_COUNTER && MCUXCL_FEATURE_CSSL_SC_USE_SW_LOCAL)
 ...
 * @endif
 */
MCUX_CSSL_FP_FUNCTION_DECL(mcuxClCss_ReleaseLock)
MCUXCLCSS_API MCUX_CSSL_FP_PROTECTED_TYPE(mcuxClCss_Status_t) mcuxClCss_ReleaseLock(
    uint32_t sessionId
    );

/**
 * @brief Check if CSS is locked
 *
 * This operation returns the locking status of CSS.
 *
 * @retval #MCUXCLCSS_STATUS_OK                  if the operation was successful and CSS is not locked
 * @retval #MCUXCLCSS_STATUS_SW_STATUS_LOCKED    if the operation was successful and CSS is locked
 *
 * @if (MCUXCL_FEATURE_CSSL_FP_USE_SECURE_COUNTER && MCUXCL_FEATURE_CSSL_SC_USE_SW_LOCAL)
 *  @return A code-flow protected error code (see @ref mcuxCsslFlowProtection). The error code can be any error code in @ref MCUXCLCSS_STATUS_, see individual documentation for more information
 * @else
 ...
 * @endif
 */
MCUX_CSSL_FP_FUNCTION_DECL(mcuxClCss_IsLocked)
MCUXCLCSS_API MCUX_CSSL_FP_PROTECTED_TYPE(mcuxClCss_Status_t) mcuxClCss_IsLocked(void);

/**
 * @brief Set the bus master ID that can unlock CSS
 *
 * This operation sets the bus master ID of the master that can override CSS lock. This command can only be executed once after reset.
 * Invalid attempts to set the bus master ID will result in a bus error.
 *
 * @param[in] masterId  The bus master identifier that can override the CSS lock. Special value #MCUXCLCSS_MASTER_UNLOCK_ANY allows any bus master identifier to override the CSS lock.
 *
 * @retval #MCUXCLCSS_STATUS_OK                   if the operation was successful
 *
 * @if (MCUXCL_FEATURE_CSSL_FP_USE_SECURE_COUNTER && MCUXCL_FEATURE_CSSL_SC_USE_SW_LOCAL)
 ...
 * @endif
 */
MCUX_CSSL_FP_FUNCTION_DECL(mcuxClCss_SetMasterUnlock)
...
/**
 * @brief Reads back the last address processed by the CSS DMA (security feature)
 *
 * @param[out] pLastAddress	Pointer to the last address read/written by the CSS DMA
 *
 * @if (MCUXCL_FEATURE_CSSL_FP_USE_SECURE_COUNTER && MCUXCL_FEATURE_CSSL_SC_USE_SW_LOCAL)
 *  @return A code-flow protected error code (see @ref mcuxCsslFlowProtection). The error code is always #MCUXCLCSS_STATUS_OK
 * @else
 ...
 * @endif
 */
MCUX_CSSL_FP_FUNCTION_DECL(mcuxClCss_GetLastDmaAddress)
MCUXCLCSS_API MCUX_CSSL_FP_PROTECTED_TYPE(mcuxClCss_Status_t) mcuxClCss_GetLastDmaAddress(
...
#ifdef MCUXCL_FEATURE_CSS_DMA_FINAL_ADDRESS_READBACK
/**
 * @brief Compares the last address processed by the CSS DMA with the expected final address of the output buffer given to the last CSS command (security feature).
 *        The given address @p outputStartAddress and expected length @p expectedLength determine the expected final address.
 *        This function can be used to verify that the final DMA transfer of a CSS command has completed as expected.
 *
 * @param[in] outputStartAddress Pointer to the output buffer of the last CSS operation
 * @param[in] expectedLength     Expected length of the output buffer of the last CSS operation
 *
 * @if (MCUXCL_FEATURE_CSSL_FP_USE_SECURE_COUNTER && MCUXCL_FEATURE_CSSL_SC_USE_SW_LOCAL)
 *  @return A code-flow protected error code (see @ref mcuxCsslFlowProtection). The error code can be any error code in @ref MCUXCLCSS_STATUS_, see individual documentation for more information
 * @else
 ...
 * @endif
 *
 * @retval #MCUXCLCSS_STATUS_SW_COMPARISON_FAILED if the comparison between the expected final address and the actual final address processed by CSS fails
 * @retval #MCUXCLCSS_STATUS_OK_WAIT              if the comparison was successful
 */
MCUX_CSSL_FP_FUNCTION_DECL(mcuxClCss_CompareDmaFinalOutputAddress)
