...
#ifndef MCUX_CL_EXAMPLE_RFC3394_HELPER_H_
#define MCUX_CL_EXAMPLE_RFC3394_HELPER_H_
...
static inline bool mcuxClExample_rfc3394_wrap(
    ...
    mcuxClCss_KeyIndex_t keyIdx,     //< keyslot index of key wrapping key
    ...
    mcuxClCss_KeyProp_t properties   //< properties of the key to be wrapped
)
{
    uint32_t concat[MCUXCLCSS_CIPHER_BLOCK_SIZE_AES/sizeof(uint32_t)] = { 0u };

    uint32_t input[MCUXCLCSS_CIPHER_BLOCK_SIZE_AES/sizeof(uint32_t)] = { 0u };
    uint8_t pKek[MCUXCLCSS_CIPHER_KEY_SIZE_AES_256] = { 0u };

    if (extkey == MCUXCLCSS_CIPHER_EXTERNAL_KEY)
    {
    ...
    for(size_t jdx = 0u; jdx < 6u; jdx++)
    {
        for(size_t idx = 0u; idx < std_n; idx++)
        {
            ...
            MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(result, token, mcuxClCss_Cipher_Async(
                cipher_options,
                keyIdx,
                pKek,
                kekLength,
...
)
{
    ...
    cipher_options.word.value   = 0;
    cipher_options.bits.cphmde  = MCUXCLCSS_CIPHERPARAM_ALGORITHM_AES_ECB;
    cipher_options.bits.dcrpt   = MCUXCLCSS_CIPHER_DECRYPT;
    cipher_options.bits.extkey  = extkey;
...
#endif /* MCUX_CL_EXAMPLE_RFC3394_HELPER_H_ */
