...
#define LIFETIME_INTERNAL  PSA_KEY_LIFETIME_FROM_PERSISTENCE_AND_LOCATION(PSA_KEY_LIFETIME_VOLATILE, PSA_KEY_LOCATION_ORACLE_MEM_STORAGE)
...
/*
 * Example of ECDSA signature generation (in the Oracle memory storage) and verification for:
 * - key size 256
 * - SHA-256
 * - signature scheme PSA_ALG_ECDSA_ANY
 */
bool mcuxClPsaDriver_ecdsa_keygen_oracleMemory_sign_verify_hash_example(void)
{
  ...
  const psa_key_attributes_t genkey_attributes = {
    .core = {                                                            // Core attributes
      ...
      .lifetime = LIFETIME_INTERNAL,                                     // Volatile (RAM), Oracle memory storage for private key
      .id = 0U,                                                          // ID zero
      .policy = {
