...
/**
 * @file  mcuxClPsaDriver_keygen_export_public_key_secpk1_example.c
 * @brief Example for SECP_K1 256bits curve key pairs generating and public exporting
 *
 * @example mcuxClPsaDriver_keygen_export_public_key_secpk1_example.c
 * @brief Example for SECP_K1 256bits curve key pairs generating and public exporting
 */
...
bool mcuxClPsaDriver_keygen_export_public_key_secpk1_example(void)
{
    /* Enable ELS */
    ...
    psa_key_attributes_t keygenAttr = {
      .private_core = {                                                               // Core attributes
        .private_type = PSA_KEY_TYPE_ECC_KEY_PAIR(PSA_ECC_FAMILY_SECP_K1),            // Keypair family with curve SECP_K1
        .private_bits = MCUXCLKEY_SIZE_256 * 8u,                                     // Key bits of SECP_K1_P256
    ...
    psa_key_attributes_t exportAttr = {
      .private_core = {                                                               // Core attributes
        .private_type = PSA_KEY_TYPE_ECC_KEY_PAIR(PSA_ECC_FAMILY_SECP_K1),            // Keypair family with curve SECP_K1
        .private_bits = MCUXCLKEY_SIZE_256 * 8u,                                     // Key bits of SECP_K1
        .private_lifetime = LIFETIME_EXTERNAL,                                        // Volatile (RAM), S50 Temporary Storage for private key
        .private_id = 0U,                                                             // ID zero
        .private_policy = {
    ...
    uint8_t data[PSA_EXPORT_PUBLIC_KEY_OUTPUT_SIZE(PSA_KEY_TYPE_ECC_PUBLIC_KEY(PSA_ECC_FAMILY_SECP_K1), MCUXCLKEY_SIZE_256 * 8u)] = {0U}; //2u * sizeof(prime_p)
