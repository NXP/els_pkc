...
bool mcuxClPsaDriver_ecdsa_sign_verify_message_example(void)
{
  ...
  const psa_key_attributes_t sign_attributes = {
    .private_core = {                                                                                                                // Core attributes
      .private_type = PSA_KEY_TYPE_ECC_KEY_PAIR(PSA_ECC_FAMILY_SECP_R1),                                                             // Curve secp256r1
      .private_bits = 256u,                                                                                                          // Key size in bits
      ...
      .private_policy = {
        .private_usage = PSA_KEY_USAGE_SIGN_MESSAGE,                                                                                 // Key may be used for signing a message
        .private_alg = PSA_ALG_ECDSA(PSA_ALG_SHA_384),                                                                               // ECDSA signature scheme without specifying a hash algorithm
  ...
  const psa_key_attributes_t verify_attributes = {
    .private_core = {                                                                                                                // Core attributes
      .private_type = PSA_KEY_TYPE_ECC_PUBLIC_KEY(PSA_ECC_FAMILY_SECP_R1),                                                           // Curve secp256r1
      .private_bits = 256u,                                                                                                          // Key bits
      ...
      .private_id = 0U,                                                                                                              // ID zero
      .private_policy = {
        .private_usage = PSA_KEY_USAGE_VERIFY_MESSAGE,                                                                               // Key may be used for verify a message
        .private_alg = PSA_ALG_ECDSA(PSA_ALG_SHA_384),                                                                               // ECDSA signature scheme without specifying a hash algorithm
        .private_alg2 = PSA_ALG_NONE},
      .private_flags = 0U},                                                                                                          // No flags
    .private_domain_parameters = NULL,
