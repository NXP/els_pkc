 ...
 *  @brief CSSv2 header for controlling the glitch detector.
 *
 * This header exposes functions that configure the CSSv2 glitch detector.
 */
 ...
 *
 * @param[in]   Pointer to the memory area which contains the glitch detector configuration. The size is fixed at
 * #MCUXCLELS_GLITCHDETECTOR_CFG_SIZE bytes.
 *
 ...
 *
 *  @return A code-flow protected error code (see @ref mcuxCsslFlowProtection). The error code can be any error code in
 * @ref MCUXCLELS_STATUS_, see individual documentation for more information
 */
...
/**
 * @brief Calculates optimal CSSv2 glitch detector configuration and writes it to system memory.
 *
 * Call #mcuxClEls_WaitForOperation to complete the operation.
 *
 * @param[out]  Pointer to the memory area which receives the glitch detector trim value. The size is fixed at
 * #MCUXCLELS_GLITCHDETECTOR_TRIM_SIZE bytes.
 *
 * @retval #MCUXCLELS_STATUS_SW_INVALID_PARAM    if invalid parameters were specified
 * @retval #MCUXCLELS_STATUS_SW_CANNOT_INTERRUPT if a running operation prevented the request
 * @retval #MCUXCLELS_STATUS_OK_WAIT             on successful request
