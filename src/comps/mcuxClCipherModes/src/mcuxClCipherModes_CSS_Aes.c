    ...
    )
{
    /* [Design]
        ...
        - Update procsssing (steps == oneshot / update)
        ...
        - Finish processing (steps == oneshot / finish)
            ...
            - If the output of the padding function is longer than zero, set cssOptions and process the blockBuffer,
    ...
    */
    ...
    /* Correct Context type for Aes CSS Skeleton */
    mcuxClCipherModes_Context_Aes_Css_t * pCtx = NULL;

    /* Correct algorithm type for Aes CSS Skeleton */
    const mcuxClCipherModes_AlgorithmDescriptor_Aes_Css_t *pAlgo = NULL;

    if ((MCUX_CL_CIPHER_OPTION_INIT == steps) || (MCUX_CL_CIPHER_OPTION_ONESHOT == steps))
    {
        if(NULL == session)
        {
            MCUX_CSSL_FP_FUNCTION_EXIT(mcuxClCipherModes_SkeletonAes, MCUX_CL_CIPHER_STATUS_ERROR);
        }

        pAlgo = pMode->pAlgorithm;

        if(MCUX_CL_CIPHER_OPTION_ONESHOT == steps)
        {
            /* Create context for Oneshot, needed by CSS */
            MCUX_CSSL_FP_FUNCTION_CALL(allocateContext, mcuxClSession_allocateCpuBuffer(session, (uint32_t **) &pCtx,
                                                                                      sizeof(mcuxClCipherModes_Context_Aes_Css_t) / sizeof(uint32_t)));
            if(MCUXCLSESSION_STATUS_OK != allocateContext)
            {
                MCUX_CSSL_FP_FUNCTION_EXIT(mcuxClCipherModes_SkeletonAes, MCUX_CL_CIPHER_STATUS_FAILURE);
            }
        ...
        {
            pCtx = (mcuxClCipherModes_Context_Aes_Css_t *) pContext;
        }
        ...
        {
            MCUX_CSSL_FP_FUNCTION_CALL(copyResult, mcuxClMemory_copy((uint8_t *) pCtx->ivState, pIv, ivLength, MCUX_CL_AES_BLOCK_SIZE));
    ...
    {
        pCtx = (mcuxClCipherModes_Context_Aes_Css_t *) pContext;
        pAlgo = (const mcuxClCipherModes_AlgorithmDescriptor_Aes_Css_t *) pCtx->common.pMode->pAlgorithm;
    }
    ...
    if ((MCUX_CL_CIPHER_OPTION_PROCESS == steps) || (MCUX_CL_CIPHER_OPTION_ONESHOT == steps))
    {
        if (pCtx->common.blockBufferUsed != 0u)
        {
            ...
            uint32_t bytesToCopy = (((MCUX_CL_AES_BLOCK_SIZE - pCtx->common.blockBufferUsed) < (inLength)) ?
                                     (MCUX_CL_AES_BLOCK_SIZE - pCtx->common.blockBufferUsed) : (inLength));
            ...
            if (MCUX_CL_AES_BLOCK_SIZE == pCtx->common.blockBufferUsed)
            {
                ...
                MCUX_CSSL_FP_FUNCTION_CALL(cipherResult, pAlgo->cryptEngine(session,
                                                                           ...
                                                                           MCUX_CL_AES_BLOCK_SIZE,
                                                                           pOutput));
                if (cipherResult != MCUX_CL_CIPHER_STATUS_OK)
                {
                    MCUX_CSSL_FP_FUNCTION_EXIT(mcuxClCipherModes_SkeletonAes, MCUX_CL_CIPHER_STATUS_ERROR);
                }
        ...
        bytesRemaining = inLength % MCUX_CL_AES_BLOCK_SIZE;
        ...
        if (inLength >= MCUX_CL_AES_BLOCK_SIZE)
        {
            ...
            MCUX_CSSL_FP_FUNCTION_CALL(cipherResult1, pAlgo->cryptEngine(session,
                                                                        ...
                                                                        bytesToProcess,
                                                                        pOutput));
            if (cipherResult1 != MCUX_CL_CIPHER_STATUS_OK)
            {
                MCUX_CSSL_FP_FUNCTION_EXIT(mcuxClCipherModes_SkeletonAes, MCUX_CL_CIPHER_STATUS_ERROR);
            }
    ...
    if ((MCUX_CL_CIPHER_OPTION_FINISH == steps) || (MCUX_CL_CIPHER_OPTION_ONESHOT == steps))
    {
        if (NULL != pAlgo->addPadding)
        {
            ...
            MCUX_CSSL_FP_FUNCTION_CALL(padResult, pAlgo->addPadding(pAlgo->blockLength,
                                                                   ...
                                                                   ((MCUX_CL_CIPHER_OPTION_ONESHOT == steps) ? inLength : 1u), /* TODO CLNS-5239: use totalInputLength from context here */
                                                                   pCtx->blockBuffer,
                                                                   &padOutLength));
        ...
        {   
            ...
            MCUX_CSSL_FP_FUNCTION_CALL(cipherResult2,  pAlgo->cryptEngine(session,
                                                                         ...
                                                                         MCUX_CL_AES_BLOCK_SIZE,
                                                                         pCtx->blockBuffer));
            if (cipherResult2 != MCUX_CL_CIPHER_STATUS_OK)
            {
                MCUX_CSSL_FP_FUNCTION_EXIT(mcuxClCipherModes_SkeletonAes, MCUX_CL_CIPHER_STATUS_ERROR);
            }
    ...
    MCUX_CSSL_FP_FUNCTION_EXIT_WITH_CHECK(mcuxClCipherModes_SkeletonAes, MCUX_CL_CIPHER_STATUS_OK, MCUX_CL_CIPHER_STATUS_FAULT_ATTACK,
        MCUX_CSSL_FP_CONDITIONAL(((MCUX_CL_CIPHER_OPTION_INIT == steps) || (MCUX_CL_CIPHER_OPTION_ONESHOT == steps)),
            MCUX_CSSL_FP_CONDITIONAL((ivLength != 0u),
                MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClMemory_copy)
            ),
            MCUX_CSSL_FP_CONDITIONAL((MCUX_CL_CIPHER_OPTION_ONESHOT == steps),
                MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClSession_allocateCpuBuffer)
            )
        ),
        MCUX_CSSL_FP_CONDITIONAL(((MCUX_CL_CIPHER_OPTION_PROCESS == steps) || (MCUX_CL_CIPHER_OPTION_ONESHOT == steps)),
            ...
            MCUX_CSSL_FP_CONDITIONAL((inLength >= MCUX_CL_AES_BLOCK_SIZE),
                pAlgo->protection_token_engine
            ),
            MCUX_CSSL_FP_CONDITIONAL((bytesRemaining != 0u),
                MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClMemory_copy))
        ),
        MCUX_CSSL_FP_CONDITIONAL(((MCUX_CL_CIPHER_OPTION_FINISH == steps) || (MCUX_CL_CIPHER_OPTION_ONESHOT == steps)),
