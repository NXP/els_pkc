...
)
{
  MCUX_CSSL_FP_FUNCTION_ENTRY(mcuxClAead_crypt);
    ...
    */
    ...
    // TODO: like this?
    mcuxClAead_Context_t ctx; // = (mcuxClAead_Context_t) mcuxClSession_allocateWords_cpuWa(session, sizeInNumberOfWords)
    ...
    MCUX_CSSL_FP_FUNCTION_CALL(ret_Skeleton, ctx.mode->pSkeleton(
    ...
    if(MCUXCLAEAD_STATUS_OK != ret_Skeleton)
    {
      MCUX_CSSL_FP_FUNCTION_EXIT(mcuxClAead_crypt, MCUXCLAEAD_STATUS_ERROR,
                                ctx.mode->protection_token_skeleton);
    }
    ...
    MCUX_CSSL_FP_FUNCTION_EXIT_WITH_CHECK(mcuxClAead_crypt, MCUXCLAEAD_STATUS_OK, MCUXCLAEAD_STATUS_FAULT_ATTACK,
                                        mode->protection_token_skeleton,
