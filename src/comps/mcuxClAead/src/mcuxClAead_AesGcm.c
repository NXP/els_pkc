    ...
    )
{
    /* [Design]
        ...
        - Common IV processing (options == oneshot / init)
            ...
            - process IV with CSS and output to state in context
...
*/
    ...
    if ((options == MCUXCLAEAD_OPTION_ONESHOT) || (options == MCUXCLAEAD_OPTION_INIT))
    {
        ...
        if (nonceLength != 12u)
        {
            ...
            bytesFullIvBlocks = (nonceLength / MCUX_CL_AES_BLOCK_SIZE) * MCUX_CL_AES_BLOCK_SIZE;
            if(0u != bytesFullIvBlocks)
            {
                MCUX_CSSL_FP_FUNCTION_CALL(retIV, pContext->mode->pEngine(session, pContext,
                                                                   pNonce,
                                                                   bytesFullIvBlocks,
                                                                   NULL,
                                                                   NULL,
            ...
            MCUX_CSSL_FP_FUNCTION_CALL(padResult, mcuxClPadding_addPadding_ISO9797_1_Method1(MCUXCLCSS_AEAD_IV_BLOCK_SIZE,
                                                                                           pNonce + bytesFullIvBlocks, nonceLength - bytesFullIvBlocks,
                                                                                           nonceLength - bytesFullIvBlocks, pContext->partialData, &padOutLength));
            ...
            MCUX_CSSL_FP_FUNCTION_CALL(ret_memset_partial, mcuxClMemory_set(pContext->partialData,
                                                                  0u,
                                                                  MCUXCLCSS_AEAD_IV_BLOCK_SIZE,
                                                                  MCUXCLCSS_AEAD_IV_BLOCK_SIZE));

            if (0u != ret_memset_partial)
            {
                MCUX_CSSL_FP_FUNCTION_EXIT(mcuxClAead_ModeSkeletonAesGcm, MCUXCLAEAD_STATUS_ERROR);
            }
            ...
            MCUX_CSSL_FP_FUNCTION_CALL(retIVFinal, pContext->mode->pEngine(session, pContext,
                                                                          pContext->partialData,
                                                                          MCUXCLCSS_AEAD_IV_BLOCK_SIZE,
                                                                          NULL,
                                                                          NULL,
                                                                          MCUXCLAEAD_ENGINE_OPTION_IV_PARTIAL_CONT | MCUXCLAEAD_ENGINE_OPTION_IV_FINAL));
        ...
        else
        {
            ...
            MCUX_CSSL_FP_FUNCTION_CALL(copyResult, mcuxClMemory_copy(pContext->partialData,
                                                                   pNonce,
                                                                   12u,
                                                                   MCUXCLCSS_AEAD_IV_BLOCK_SIZE));
...
*/
    ...
    if ((options == MCUXCLAEAD_OPTION_ONESHOT) || (options == MCUXCLAEAD_OPTION_PROCESS_AAD))
    {
        ...
        bytesToCopy = (((MCUXCLCSS_AEAD_AAD_BLOCK_SIZE - pContext->partialDataLength) < (adataLength)) ?
                                (MCUXCLCSS_AEAD_AAD_BLOCK_SIZE - pContext->partialDataLength) : (adataLength));

        if (pContext->partialDataLength != 0u)
        {
            aadProFPFlag1 = 1u;
            /* Add new data into the partial data buffer and process, if possible. */
            MCUX_CSSL_FP_FUNCTION_CALL(copyResult, mcuxClMemory_copy(pContext->partialData + pContext->partialDataLength,
                                                                   pAdata,
                                                                   bytesToCopy,
            ...
            if (MCUXCLCSS_AEAD_AAD_BLOCK_SIZE == pContext->partialDataLength)
            {
                aadProFPFlag2 = 1u;
                /* partialData now contains a full block, so process it. */
                MCUX_CSSL_FP_FUNCTION_CALL(retAad, pContext->mode->pEngine(session, pContext,
                                                                   pContext->partialData,
                                                                   MCUXCLCSS_AEAD_AAD_BLOCK_SIZE,
                                                                   NULL,
                                                                   NULL,
        ...
        bytesRemainingAad = (adataLength / MCUXCLCSS_AEAD_AAD_BLOCK_SIZE) * MCUXCLCSS_AEAD_AAD_BLOCK_SIZE;
        if(0u != bytesRemainingAad)
        {
            /* Process as many remaining full blocks as possible. */
            MCUX_CSSL_FP_FUNCTION_CALL(retBlkAad, pContext->mode->pEngine(session, pContext,
                                                                         pAdata + bytesCopied,
                                                                         bytesRemainingAad,
                                                                         NULL,
                                                                         NULL,
        ...
        if((pContext->processedDataLength == pContext->aadLength) && (0u != pContext->partialDataLength))
        {
            ...
            MCUX_CSSL_FP_FUNCTION_CALL(ret_zeropad, mcuxClMemory_set(pContext->partialData + pContext->partialDataLength,
                                                                0u,
                                                                MCUXCLCSS_AEAD_AAD_BLOCK_SIZE - pContext->partialDataLength,
                                                                MCUXCLCSS_AEAD_AAD_BLOCK_SIZE));
            ...
            MCUX_CSSL_FP_FUNCTION_CALL(retPadAad, pContext->mode->pEngine(session, pContext,
                                                                  pContext->partialData,
                                                                  MCUXCLCSS_AEAD_AAD_BLOCK_SIZE,
                                                                  NULL,
                                                                  NULL,
...
*/
    ...
    if ((options == MCUXCLAEAD_OPTION_ONESHOT) || (options == MCUXCLAEAD_OPTION_PROCESS))
    {
        ...
        bytesCopied = 0u;
        bytesToCopy = (((MCUX_CL_AES_BLOCK_SIZE - pContext->partialDataLength) < (inLength)) ?
                        (MCUX_CL_AES_BLOCK_SIZE - pContext->partialDataLength) : (inLength));
        ...
        if (pContext->partialDataLength != 0u)
        {
            ...
            if (MCUX_CL_AES_BLOCK_SIZE == pContext->partialDataLength)
            {
                dataProFPFlag2 = 1u;
                /* partialData now contains a full block, so process it. */
                MCUX_CSSL_FP_FUNCTION_CALL(retData, pContext->mode->pEngine(session, pContext,
                                                                             pContext->partialData,
                                                                             MCUX_CL_AES_BLOCK_SIZE,
                                                                             pOutput,
                                                                             NULL,
        ...
        bytesRemainingData = (inLength / MCUX_CL_AES_BLOCK_SIZE) * MCUX_CL_AES_BLOCK_SIZE;
        if(0u != bytesRemainingData)
        {
            /* Process as many remaining full blocks as possible. */
            MCUX_CSSL_FP_FUNCTION_CALL(retBlkData, pContext->mode->pEngine(session, pContext,
                                                                     pIn + bytesCopied,
                                                                     bytesRemainingData,
                                                                     pOutput,
                                                                     NULL,
        ...
        if((pContext->processedDataLength == pContext->dataLength + pContext->aadLength) && (0u != pContext->partialDataLength))
        {
            ...
            MCUX_CSSL_FP_FUNCTION_CALL(redFinalData, pContext->mode->pEngine(session, pContext,
                                                                     pContext->partialData,
                                                                     MCUX_CL_AES_BLOCK_SIZE,
                                                                     pOutput,
                                                                     NULL,
...
*/
    ...
    if ((options == MCUXCLAEAD_OPTION_ONESHOT) || (options == MCUXCLAEAD_OPTION_FINISH) || (options == MCUXCLAEAD_OPTION_VERIFY))
    {
        ...
        MCUX_CSSL_FP_FUNCTION_CALL(retFinal, pContext->mode->pEngine(session, pContext,
                                                                   NULL,
                                                                   NULL,
                                                                   pContext->partialData,
                                                                   NULL,
...
*/
    ...
    {
        MCUX_CSSL_FP_FUNCTION_CALL(copyResult, mcuxClMemory_copy(pTag,
                                                               pContext->partialData,
                                                               tagLength,
                                                               tagLength));
...
*/
    ...
    MCUX_CSSL_FP_FUNCTION_EXIT_WITH_CHECK(mcuxClAead_ModeSkeletonAesGcm, MCUXCLAEAD_STATUS_OK, MCUXCLAEAD_STATUS_FAULT_ATTACK,
        MCUX_CSSL_FP_CONDITIONAL(((options == MCUXCLAEAD_OPTION_ONESHOT) || (options == MCUXCLAEAD_OPTION_INIT)),
            MCUX_CSSL_FP_CONDITIONAL((nonceLength != 12u), MCUX_CSSL_FP_CONDITIONAL((0u != bytesFullIvBlocks), pContext->mode->protection_token_engine),
        ...
        MCUX_CSSL_FP_CONDITIONAL((((options == MCUXCLAEAD_OPTION_ONESHOT) || (options == MCUXCLAEAD_OPTION_PROCESS_AAD))),
            MCUX_CSSL_FP_CONDITIONAL((aadProFPFlag1 == 1u), MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClMemory_copy),
                MCUX_CSSL_FP_CONDITIONAL((aadProFPFlag2 == 1u), pContext->mode->protection_token_engine)
            ),
            MCUX_CSSL_FP_CONDITIONAL((0 != ((adataLength / MCUXCLCSS_AEAD_AAD_BLOCK_SIZE) * MCUXCLCSS_AEAD_AAD_BLOCK_SIZE)), pContext->mode->protection_token_engine),
        ...
        MCUX_CSSL_FP_CONDITIONAL((((options == MCUXCLAEAD_OPTION_ONESHOT) || (options == MCUXCLAEAD_OPTION_PROCESS))),
            MCUX_CSSL_FP_CONDITIONAL((dataProFPFlag1 == 1u), MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClMemory_copy),
                MCUX_CSSL_FP_CONDITIONAL((dataProFPFlag2 == 1u), pContext->mode->protection_token_engine)
            ),
            MCUX_CSSL_FP_CONDITIONAL((0 != ((inLength / MCUX_CL_AES_BLOCK_SIZE) * MCUX_CL_AES_BLOCK_SIZE)), pContext->mode->protection_token_engine),
