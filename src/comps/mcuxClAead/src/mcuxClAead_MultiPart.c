...
)
{
  MCUX_CSSL_FP_FUNCTION_ENTRY(mcuxClAead_init);
    ...
    */
    pContext->mode = mode;
    pContext->key = key;
    pContext->dataLength = inLength;
    pContext->aadLength = adataLength;
    pContext->tagLength = tagLength;
    pContext->processedDataLength = 0u;

    MCUX_CSSL_FP_FUNCTION_CALL(ret_Skeleton, pContext->mode->pSkeleton(
    ...
    if(MCUXCLAEAD_STATUS_OK != ret_Skeleton)
    {
       MCUX_CSSL_FP_FUNCTION_EXIT(mcuxClAead_init, MCUXCLAEAD_STATUS_ERROR,
                                 pContext->mode->protection_token_skeleton);
    }
...
)
{
    ...
    /* [Design]
        ...
        - call the Skeleton with
            ...
            - unused arguments = NULL/0
    */
    if((pContext->processedDataLength < pContext->aadLength) ||
       ((pContext->processedDataLength + inLength) > (pContext->aadLength + pContext->dataLength)))
    {
  ...
  MCUX_CSSL_FP_FUNCTION_ENTRY(mcuxClAead_process_adata);
    /* [Design]
        ...
        - call the Skeleton with
            ...
            - unused arguments = NULL/0
    */
    if((pContext->processedDataLength + adataLength) > pContext->aadLength)
    {
...
)
{
  MCUX_CSSL_FP_FUNCTION_ENTRY(mcuxClAead_finish);
    ...
    */
    ...
    if(pContext->processedDataLength != (pContext->dataLength + pContext->aadLength))
    {
    ...
    MCUX_CSSL_FP_FUNCTION_CALL(ret_Skeleton, pContext->mode->pSkeleton(
      ...
      /* uint32_t tagLength,                   */ pContext->tagLength,
