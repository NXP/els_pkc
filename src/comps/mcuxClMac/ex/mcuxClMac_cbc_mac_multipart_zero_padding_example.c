...
bool mcuxClMac_cbc_mac_multipart_zero_padding_example(void)
{
    ...
    uint32_t key_buffer[MCUX_CL_KEY_SIZE_256 / sizeof(uint32_t)];
    ...
    // process
    // finish
    uint8_t context[MCUXCLMAC_CMAC_CTX_SIZE];
    mcuxClMac_Context_t * ctx = (mcuxClMac_Context_t *) context;

    /* Call the mcuxClMac_init */
    MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(initResult, initToken, mcuxClMac_init(&session,
                                                                    ctx,
                                                                    key,
                                                                    mcuxClMac_Mode_CBCMAC_PaddingISO9797_1_Method1));

    if((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClMac_init) != initToken) || (MCUXCLMAC_ERRORCODE_OK != initResult))
    {
        return MCUX_CL_EXAMPLE_FAILURE;
    }
    ...
    if((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClMac_process) != processToken) || (MCUXCLMAC_ERRORCODE_OK != processResult))
    {
        return MCUX_CL_EXAMPLE_FAILURE;
    }
    MCUX_CSSL_FP_FUNCTION_CALL_END();
    ...
    if((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClMac_finish) != finishToken) || (MCUXCLMAC_ERRORCODE_OK != finishResult))
    {
        return MCUX_CL_EXAMPLE_FAILURE;
    }
    MCUX_CSSL_FP_FUNCTION_CALL_END();
