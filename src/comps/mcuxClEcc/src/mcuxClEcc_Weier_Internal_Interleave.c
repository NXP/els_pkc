...
/**
 * @file  mcuxClEcc_Weier_Internal_Interleave.c
 * @brief mcuxClEcc: implementation of ECC function mcuxClEcc_Interleave
 */
...
#include <internal/mcuxClEcc_Weier_Internal_Interleave_FUP.h>
 ...
 *   offsets pOperands[VX0/VY0/VZ] are modified;
...
MCUX_CSSL_FP_PROTECTED_TYPE(mcuxClEcc_Status_t) mcuxClEcc_Interleave(uint16_t iScalar0_iScalar1, uint32_t scalarBitLength)
{
    ...
    uint32_t offsets_VY0_VX0 = /* VX0 */ (uint32_t) pOperands[iScalar0]
                               /* VY0 */ + (((uint32_t) pOperands[ECC_T0] + byteLenHalfScalar_PKCWord) << 16);

    MCUXCLPKC_WAITFORREADY();
    MCUXCLECC_STORE_2OFFSETS(pOperands32, WEIER_VX0, WEIER_VY0, offsets_VY0_VX0);
    pOperands[WEIER_VZ] = (uint16_t) (0u - bitLenHalfScalar);  /* PKC will ignore higher bits of shifting amount. */
    ...
    /* Interleave scalar0. */
    MCUXCLPKC_FP_CALCFUP(mcuxClEcc_FUP_Weier_Interleave, mcuxClEcc_FUP_Weier_Interleave_LEN);

    /* Interleave scalar1. */
    MCUXCLPKC_WAITFORREADY();
    pOperands[WEIER_VX0] = pOperands[iScalar1];
    MCUXCLPKC_FP_CALCFUP(mcuxClEcc_FUP_Weier_Interleave, mcuxClEcc_FUP_Weier_Interleave_LEN);
